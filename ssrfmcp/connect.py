"""Native cognis-connect emit for ssrfmcp — forward findings to any platform.

Maps ssrfmcp's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ssrfmcp ... --format json | ssrfmcp-emit --to stix
    ssrfmcp-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('target', '') + ' SSRF Probe'
        out['severity'] = rec.get('severity', 'info')
        out['type'] = 'SSRF'
        out['description'] = f"SSRF vulnerability detected in {rec.get('target', '')}"
